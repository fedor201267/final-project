# final-project
## my idea for this project
in visual studio code make a bot that can have a few comands that he replies on. *TRY* to educate techable machine so when user sends specific image it can anwser.
1. /hello
2. /images
3. /ecological_advises
4. ![stop-pollution-sign-smokestacks-environmental-pollution-poster-vector-illustration-62958364](https://github.com/user-attachments/assets/325c1ccc-0678-437f-954a-84bd1713ec9a)  *avatar pick*.
